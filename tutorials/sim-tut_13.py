...
def decorate_and_save_plot(
    fig, ax, fname, title, xlab, ylab, xlims, ylims, legend_loc=None, legend_ncol=1
):
    ax.set_xlabel(xlab)
    ax.set_ylabel(ylab)
...
def solve_elastic_rod_analytical(prefix, qvec, nmodes, coremat, arad):
    ...
    launch_worker_threads_and_wait(num_cores, solelrod_callee, q_result, q_work)
    ...
    neff_an = (
        Vref * qvec[:, np.newaxis] / (nu_an * twopi + 1e-5)
    )  # small bit to avoid div by zero
    ...
    for p in range(plo, phi + 1):
        ...
        if False:  # individual dispersion plots if desired
            ...
            decorate_and_save_plot(
                figb,
                axb,
                prefix + "-acdisp_qnu-p%d.png" % p,
                "Dispersion p=%d" % p,
                r"$q\, [\mathrm{μm}^{-1}]$ ",
                r"$\nu$ [GHz]",
                (0, qvec[-1] * 1e-6),
    ...
    ax.plot(qvec * 1e-6, qvec * Vl / (twopi * 1e9), ":", color="gray", lw=0.5)
    ax.plot(qvec * 1e-6, qvec * Vs / (twopi * 1e9), ":", color="gray", lw=0.5)
    decorate_and_save_plot(
        fig,
        ax,
        prefix + "-acdisp_qnu_exact.png",
...
def solve_elastic_rod_numerical(prefix, qvec, nmodes, wguide, simres_EM, cmat):
    ...
    def accalc_callee(args):
        (iq, tq, doplot, wg) = args  # Matches queues passed to CalcThread

        shift_Hz = (
            0.8 * tq * Vs / twopi
        )  # look for modes not to far from the shear sound line
        simres_AC = wg.calc_AC_modes(
            nbasis_AC, tq, shift_Hz=shift_Hz, EM_sim=simres_EM
        )  # , bcs='Open')
        v_nu_num = np.sort(np.real(simres_AC.nu_AC_all()))[:nmodes]

        if doplot:  # Only worker 1 will ever do this
            print(
                "{0} is plotting elastic modes at iq = {1:d} of [0..{2:d}].".format(
    ...
    num_cores = 0
    launch_worker_threads_and_wait(num_cores, accalc_callee, q_result, q_work, q_work_plot)

    # TODO: this is hanging with more than one process
    # Matplotlib can't be called from child threads so must use processes
    #num_cores = 2
    #launch_worker_processes_and_wait( num_cores, accalc_callee, q_result, q_work, q_work_plot)
    ...
    Vref = Vs
    neff_num = (
        Vref * qvec[:, np.newaxis] / (nu_num * twopi + 1e-5)
    )  # small bit to avoid div by zero
...
def solve_elastic_dispersion(prefix, ssys, wguide, simres_EM, rcore, mat_core):
    ...
    nu_num = solve_elastic_rod_numerical(
        prefix, qvec_num, nmodes, wguide, simres_EM, mat_core
    )
    ...
    neff_an = (
        Vref * qvec_an[:, np.newaxis] / (nu_an * twopi + 1e-5)
    )  # avoid div by zero

    for ip in range(nu_an.shape[0]):
        if ip == 0:
            lab = r"$p=0$, Torsional"  # Indices different because of 2 p=0 cases pushing columns to right
        elif ip == 1:
            lab = r"$p=0$, Poch."
        else:
            lab = r"$p=%d$" % (ip - 1)
        ...
        np.savetxt(
            prefix + "-acdisp_qnu_p%d.txt" % (ip - 1),
            mdata,
            fmt="%.8e",
...
def do_main():
    ...
    wguide = nbapp.make_structure(
        ...
        material_a=mat_core,
        lc_bkg=0.1,
        lc_refine_1=3.0 * refine_fac,
        lc_refine_2=3 * refine_fac,
    )

    #prefix = pref0
    # wguide.plot_mesh(prefix)

    # move to solve_elastic_rod_numerical
    # solve one EM step to prep the waveguide meshing
    simres_EM = wguide.calc_EM_modes(40, 1550, 1.5)
