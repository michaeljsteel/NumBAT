...
#    linewidth_Hz = alpha/np.pi # SBS linewidth of each resonance in [Hz]
    ...
    if struc.using_linear_elements():
        Q_PE = NumBAT.photoelastic_int_v2(
            ...
            elastic_props.n_mats_ac, elastic_props.p_ijkl,
    ...
    Q_MB = NumBAT.moving_boundary(sim_EM_pump.n_modes, sim_EM_Stokes.n_modes,
            ...
            elastic_props.n_mats_ac, typ_select_in, typ_select_out,
...
def interp_py_fields(sim_EM_pump, sim_EM_Stokes, sim_AC, q_AC, n_points,
...
def grid_integral(m_n, sim_AC_structure, sim_AC_Omega_AC, n_pts_x, n_pts_y,
    ...
    for i in range(3):
        for k in range(3):
            for l in range(3):
                for j in range(3):
                    integrand = del_u_mat[i,j]*del_u_mat_star[k,l]*sim_AC_structure.elastic_props.eta_ijkl[i,j,k,l]
                    I = np.zeros( n_pts_x )
                    for r in range(n_pts_x):
        ...
        for k in range(3):
            for l in range(3):
                for j in range(3):
                    ...
                    integrand_PE = m_n**4 * E_mat_p[j]*np.conj(E_mat_S[i])*sim_AC_structure.elastic_props.p_ijkl[i,j,k,l]*del_u_mat_star[k,l]
                    I = np.zeros( n_pts_x )
                    for r in range(n_pts_x):
...
def gain_python(sim_EM_pump, sim_EM_Stokes, sim_AC, q_AC, comsol_data_file, comsol_ivals=1):
