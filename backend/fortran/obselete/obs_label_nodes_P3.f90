...
subroutine label_nodes_P3 (nel, npt, nnodes, n_edge,&
   npt_p3, m_elnd_to_mshpt, table_N_E_F, visited)
   ...
   integer(8) m_elnd_to_mshpt(nnodes,nel), table_N_E_F(14,nel)
   ...
   do iel=1,nel
      ...
      do inod=4,6
         ...
         j1 = inod - 3
         vert_1(1) = m_elnd_to_mshpt(j1,iel)
         j1 = inod - 2
         if (j1 .gt. 3) j1 = j1 - 3
         vert_1(2) = m_elnd_to_mshpt(j1,iel)
         ...
         if(visited(k) .eq. 0) then
            ...
            do j=4,6
               ...
               if (k .eq. j1) then
                  ...
                  !  Vertices of the edge
                  j1 = inod2 - 3
                  vert_2(1) = m_elnd_to_mshpt(j1,iel2)
                  j1 = inod2 - 2
                  if (j1 .gt. 3) j1 = j1 - 3
                  vert_2(2) = m_elnd_to_mshpt(j1,iel2)
               endif
            ...
            do j=1,2
               ...
               elseif (vert_2(1) .eq. vert_1(2) .and.  vert_2(2) .eq. vert_1(1)) then
                  ...
                  write(*,*) "inod, m_elnd_to_mshpt(inod,iel) = ", inod, &
                     m_elnd_to_mshpt(inod,iel)
                  write(*,*) "inod2, m_elnd_to_mshpt(inod2,iel2) = ", inod2, &
                     m_elnd_to_mshpt(inod2,iel2)
