...
&n_period_N_E_F, lat_vecs)
   ...
   !***********************************************************************
   !
   !   type_N_E_F(1,i) = 0  => interiour point
   !   type_N_E_F(1,i) != 0 => boundary point
   !   type_N_E_F(2,i)  = 0, 1, 2 => diemnsion of the domain
   !
   !
   !***********************************************************************
   !
...
10             continue
               ...
               if(test_lattice .eq. 1) then
                  ...
                  elseif(i1 .ne. 0 .and. j1 .eq. 0) then
                     ...
                     stop
                     !                  ip_period_N_E_F(i) = min(i,i1)
                     !                  ip_period_N_E_F(j) = min(i,i1)
                  endif
               endif
            endif
         enddo
