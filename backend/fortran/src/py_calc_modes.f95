...
!
   integer*8 n_msh_el, n_msh_pts, nodes_per_el, ui, bdy_cdn
!
...
!
   ...
   call geometry (n_msh_el, n_msh_pts, nodes_per_el, n_typ_el, dim_x, dim_y, type_nod, &
      type_el, table_nod, mesh_xy, mesh_file, errco, emsg)
   RETONERROR(errco)
...
!
   ...
   call list_edge (n_msh_el, n_msh_pts, nodes_per_el, n_edge, type_nod, table_nod, &
      a_iwork(ip_table_E), a_iwork(ip_table_N_E_F), a_iwork(ip_visite))
   call list_node_P3 (n_msh_el, n_msh_pts, nodes_per_el, n_edge, n_msh_pts_p3, table_nod, &
      a_iwork(ip_table_N_E_F), a_iwork(ip_visite))
...
!       !  Periodic  conditions (never in NumBAT)
!    elseif( bdy_cdn .eq. BCS_PERIODIC) then
!       if (debug .eq. 1) then
...
!       call periodic_N_E_F (n_ddl, a_iwork(ip_type_N_E_F), b_zwork(jp_x_N_E_F), a_iwork(ip_period_N_E_F), &
...
!    write(ui,*) "py_calc_modes.f: neq, n_ddl = ", neq, n_ddl
   !
   !=====calcul du vecteur de localisation des colonnes
   !  pour le traitement skyline de la matrice globale
   !  Type of sparse storage of the global matrice:
   !     Symmetric Sparse Skyline format
!  Determine the pointer for the Symmetric Sparse Skyline format
!
!  ip_col_ptr = ip_eq + 3*n_ddl
!  ip_bandw  = ip_col_ptr + neq + 1
!  int_used = ip_bandw + neq + 1
!C
...
!  endif
      ...
      call asmbly ( bdy_cdn, i_base, n_msh_el, n_msh_pts, n_ddl, neq, nodes_per_el, shift_ksqr, &
         ...
         a_iwork(ip_period_N), a_iwork(ip_period_N_E_F), mesh_xy, b_zwork(jp_x_N_E_F), nonz, a_iwork(ip_row), &
...
!  using the permutation vector iindex
      call array_sol ( bdy_cdn, n_modes, n_msh_el, n_msh_pts, n_ddl, neq, nodes_per_el, n_core, &
         ...
         a_iwork(ip_period_N_E_F), mesh_xy, b_zwork(jp_x_N_E_F), beta, mode_pol, b_zwork(jp_vp), sol)
!
      if(debug .eq. 1) then
...
!  *  a_iwork(ip_visite), gmsh_file_pos, dir_name,
