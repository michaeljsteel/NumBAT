...
subroutine build_mesh_tables( n_msh_el, n_msh_pts, nodes_per_el, n_ddl, &
   ...
   !  Fills:  tab_N_E_F[1,:]
   call label_faces (mesh_raw%n_msh_el, entities%elnd_to_mesh)
   ...
   call label_nodes_P3 (n_msh_el, n_msh_pts, nodes_per_el, n_edge, n_msh_pts_p3, mesh_raw%elnd_to_mesh, &
      entities%elnd_to_mesh,  visited)
   ...
   call type_node_edge_face (n_msh_el, n_msh_pts, nodes_per_el, n_ddl, mesh_raw%type_nod, mesh_raw%elnd_to_mesh, &
      entities%elnd_to_mesh, visited , entities%type_nod, mesh_raw%v_nd_xy, entities%v_nd_xy )


   !  Fills: entities%type_nod(1:2, 1:n_ddl), x_E_F(1:2, 1:n_ddl)
   call get_coord_p3 (n_msh_el, n_msh_pts, nodes_per_el, n_ddl, mesh_raw%elnd_to_mesh, mesh_raw%type_nod, &
      entities%elnd_to_mesh, entities%type_nod, mesh_raw%v_nd_xy, entities%v_nd_xy , visited)
