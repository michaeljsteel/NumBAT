...
subroutine build_mesh_tables( n_msh_el, n_msh_pts, nodes_per_el, n_ddl, &
   ...
   !  Fills:  tab_N_E_F[1,:]
   call label_faces (mesh_raw%n_msh_el, entities%table_nod)
   ...
   call label_nodes_P3 (n_msh_el, n_msh_pts, nodes_per_el, n_edge, n_msh_pts_p3, mesh_raw%table_nod, &
      entities%table_nod,  visited)
   ...
   call type_node_edge_face (n_msh_el, n_msh_pts, nodes_per_el, n_ddl, mesh_raw%type_nod, mesh_raw%table_nod, &
      entities%table_nod, visited , entities%type_nod, mesh_raw%xy_nodes, entities%xy_nodes )


   !  Fills: entities%type_nod(1:2, 1:n_ddl), x_E_F(1:2, 1:n_ddl)
   call get_coord_p3 (n_msh_el, n_msh_pts, nodes_per_el, n_ddl, mesh_raw%table_nod, mesh_raw%type_nod, &
      entities%table_nod, entities%type_nod, mesh_raw%xy_nodes, entities%xy_nodes , visited)
