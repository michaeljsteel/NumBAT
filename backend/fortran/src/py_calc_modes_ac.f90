...
subroutine calc_AC_modes( n_modes, q_ac, dimscale_in_m, shift_nu, &
    ...
    !         type_el:
    !         x_arr
    !         v_nu_out:  eigen frequencies nu=omega/(2D_PI) for each mode
    !         sol1:
    !         mode_pol:
    ...
    complex*16, target :: v_nu_out(n_modes)
    ...
    !f2py intent(in) type_nod, table_nod, type_el, x_arr, symmetry_flag
    ...
    !f2py depend(x_arr) n_msh_pts

    !f2py intent(out) v_nu_out
    !f2py intent(out) sol1, mode_pol, table_nod, type_el, x_arr
    ...
    if (supplied_geo_flag .eq. 0) then
        call geometry (n_msh_el, n_msh_pts, nodes_per_el, n_typ_el, dim_x, dim_y,  &
            mesh_file, x_arr, type_nod, type_el, table_nod, errco, emsg)
        if (errco .ne. 0) then
            return
        endif
    endif
    ...
    call asmbly_AC (i_base, n_msh_el, n_msh_pts, neq, nodes_per_el, &
        ...
        x_arr, nonz, a_iwork(ip_row), a_iwork(ip_col_ptr), &
    ...
    call valpr_64_AC (i_base, nvect, n_modes, neq, itermax, ltrav, &
        ...
        b_zwork(jp_vschur), v_nu_out, b_zwork(jp_trav), b_zwork(jp_vp), &
    ...
    enddo
    !
    call z_indexx_AC (n_modes, v_nu_out, iindex)
    !
    ...
    call array_sol_AC (n_modes, n_msh_el, n_msh_pts, neq, &
        nodes_per_el, iindex, table_nod, type_el, a_iwork(ip_eq), x_arr, &
        v_nu_out,  b_zwork(jp_eigenum_modes_tmp), mode_pol, b_zwork(jp_vp), sol1)
    ...
    !     *         x_arr, v_nu_out, sol1, b_zwork(jp_rhs), a_iwork(ip_visite),
