...
subroutine label_edges (mesh_raw, table_nod, n_edge, visited)
   ...
   integer(8) table_nod(14,mesh_raw%n_msh_el)
...
!            if (type_nod(table_nod(j,i)) .ne. 0) then  ! edge node is on a physical bdy
            ...
            if (.not. mesh_raw%is_boundary_node_2(j1,i) .or. .not. mesh_raw%is_boundary_node_2(j2,i)) then
               ...
               write(*,*) "type_nod(j1) = ", mesh_raw%table_nod(j1,i)
               write(*,*) "type_nod(j2) = ", mesh_raw%table_nod(j2,i)
               write(*,*) "type_nod(j) = ", mesh_raw%table_nod(j,i)
      ...
      do j=4,mesh_raw%nodes_per_el
         ...
         j1 = mesh_raw%table_nod(j,i)  ! find the node
         k = visited(j1)
         ...
         if (k .eq. 0) then        ! a new edge encountered
            ...
            j2 = list_end(1,j-3)
            table_edge(1,n_edge) = mesh_raw%table_nod(j2,i)

            j2 = list_end(2,j-3)
            table_edge(2,n_edge) = mesh_raw%table_nod(j2,i)
            table_edge(3,n_edge) = j1

            !  Table of connectivity for the face (with respect to the triangle element)
            table_nod(j-2,i) = n_edge + n_face
            table_edge(4,n_edge) = n_edge + n_face

         else

            table_nod(j-2,i) = k + n_face
