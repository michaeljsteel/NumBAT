...
def elasticfreeslab_Lamb_chareq_even(Omega: complex | float, Vs: float, Vl: float, wid: float, q: float) -> float:
...
def elasticfreeslab_Lamb_chareq_odd(Omega: complex | float, Vs: float, Vl: float, wid: float, q: float) -> float:
    """Dispersion relation for TE slab optical waveguide in normalised units."""
    ONE = 1.0 + 0.0j
    kappa_s = np.sqrt(ONE * (Omega / Vs) ** 2 - q**2)
    kappa_l = np.sqrt(ONE * (Omega / Vl) ** 2 - q**2)
