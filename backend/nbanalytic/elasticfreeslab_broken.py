...
def ombrak_is_at_tan_resonance(ombrak: Sequence[float], q: float, Vs: float, Vl: float, width: float) -> bool:
    if (ombrak[0] / Vs) ** 2 - q**2 > 0:
        sarg0 = np.sqrt((ombrak[0] / Vs) ** 2 - q**2) * width / 2 / (np.pi / 2)

        sarg1 = np.sqrt((ombrak[1] / Vs) ** 2 - q**2) * width / 2 / (np.pi / 2)

        if math.ceil(sarg0) == math.floor(sarg1) and math.ceil(sarg0) % 2 == 1:  # an odd multiple of pi/2
                return True

    if (ombrak[0] / Vl) ** 2 - q**2 > 0:
        larg0 = np.sqrt((ombrak[0] / Vl) ** 2 - q**2) * width / 2 / (np.pi / 2)

        larg1 = np.sqrt((ombrak[1] / Vl) ** 2 - q**2) * width / 2 / (np.pi / 2)
...
def qbrak_is_at_tan_resonance(Omega: float, qbrak: Sequence[float], Vs: float, Vl: float, width: float) -> bool:
    if (Omega / Vs) ** 2 - qbrak[1] ** 2 > 0:
        sarg0 = np.sqrt((Omega / Vs) ** 2 - qbrak[0] ** 2) * width / 2 / (np.pi / 2)
        sarg1 =  np.sqrt((Omega / Vs) ** 2 - qbrak[1] ** 2) * width / 2 / (np.pi / 2)

        if math.ceil(sarg0) == math.floor(sarg1) and math.ceil(sarg0) % 2 == 1 :  # an odd multiple of pi/2
            return True

    if (Omega / Vl) ** 2 - qbrak[1] ** 2 > 0:
        larg0 = np.sqrt((Omega / Vl) ** 2 - qbrak[0] ** 2) * width / 2 / (np.pi / 2)
        larg1 = np.sqrt((Omega / Vl) ** 2 - qbrak[1] ** 2) * width / 2 / (np.pi / 2)
...
class ElasticIsotropicFreeSlab:
    ...
    def __init__(
        ...
        self._mats = material  # slab material
        ...
        self.width = thickness_SI/g_norms.x0
    ...
    def _find_Lamb_q_brackets_smart(self, Omega: float, drfunc: Callable[[float], float]) -> List[Tuple[float, float]]:
        ...
        hi_ns = np.ceil(self.width * Omega / (2 * np.pi * self._Vbulk_shear) - 0.5)
        hi_nl = np.ceil(self.width * Omega / (2 * np.pi * self._Vbulk_long) - 0.5)

        bad_qs = (
            (Omega / self._Vbulk_shear) ** 2
            - ((np.arange(0, hi_ns) + 0.5) * 2 * np.pi / self.width) ** 2
        ) ** 0.5
        bad_ql = (
            (Omega / self._Vbulk_long) ** 2
    ...
    def _find_Lamb_q_brackets(self, Omega: float, drfunc: Callable[[float], float],
                              q_bracket_steps: int = 1000) -> List[Tuple[float, float]]:
        # brute force bracketing
        qsteps = q_bracket_steps

        qlo = 1e-9
        qhi = Omega / self._Vbulk_shear * 4
        v_q = np.linspace(qlo, qhi, qsteps)
        ...
        for i in range(1, len(v_disprel) - 1):
            ...
            if fi * fi_m1 < 0:  # a possible crossing?
                if abs(fi) < is_small and abs(fi_m1) < is_small:  # legit
                    ...
                    if not qbrak_is_at_tan_resonance(Omega, qbrak, self._Vbulk_shear, self._Vbulk_long, self.width):
                        qbraks.append(qbrak)
        ...
        for i in range(i_st + 1, len(v_disprel) - 1):
            ...
            if fi * fi_m1 < 0:  # a possible crossing?
                if abs(fi) < is_small and abs(fi_m1) < is_small:  # legit
                    ...
                    if not ombrak_is_at_tan_resonance(ombrak, q, self._Vbulk_shear, self._Vbulk_long, self.width):
    ...
    #    vl = self._Vbulk_long / self._Vbulk_shear
#
#        return vR**6 - 8 * vR**4 + vR**2 * (24 - 16 / vl**2) + 16 * (1 / vl**2 - 1)
    ...
    def find_Rayleigh_dispersion(self, v_Omega: Sequence[float], col_array: bool = False) -> Tuple[NDArray, Optional[NDArray]]:
        ...
        ## Calculation works in units of self._Vbulk_shear
        #vRlo = 0.001
        #vRhi = 1
        ...
        ## put back units of self._Vbulk_shear
        #v_qR = v_Omega / (vR * self._Vbulk_shear)
        return v_qR, v_col
