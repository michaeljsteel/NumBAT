...
# allow this function to be module leve rather than class-level
        ...
        if isinstance(EM_AC, str):  # aim to get rid of this
            if self.sim.is_AC():
                ...
                try:
                    ...
                    raise ValueError(
                        "EM_AC must be either 'AC', 'EM_E' or 'EM_H'.") from ex
    ...
    def setup_plot_grid(self, n_points=501,  # quiver_points=30,
        ...
        for i_el in np.arange(sim.n_msh_el):
            for i_node in np.arange(6):
                ...
                # Fact that this is mesh_xy and not y_arr seems to be right
...
class Mode(object):
    ...
    def analyse_mode(self, n_points=501, EM_field=FieldType.EM_E):
        ...
        # TODO: make all this cleaner. this should happen once for all modes
        ...
        if len(comps):
            ...
            for comp in comps:
                ...
                plotting.plot_one_component(
                    mh.m_X, mh.m_Y, v_plots, mh.plot_params, self.sim, self.mode_num, cc, ax)
    ...
    def _analyse_mode(self, v_x, v_y, m_Refx, m_Refy, m_Refz, m_Imfx, m_Imfy, m_Imfz, m_Absf):
        ...
        # TODO: IS THIS RIGHT?
        [m_x, m_y] = np.meshgrid(v_x, v_y, indexing='ij')
        ...
        m_mod = m_Refx*m_Refx+m_Imfx*m_Imfx+m_Refy * \
            m_Refy+m_Imfy*m_Imfy+m_Refz*m_Refz+m_Imfz*m_Imfz
...
class Simulation(object):
    ...
    def __init__(self, structure, num_modes=20, wl_nm=1, n_eff=None, shift_Hz=None,
        ...
        v_refindexn = []
        v_refindexn_tmp = np.array(
            [m.refindex_n for m in self.structure.d_materials.values()])
    ...
    def calc_EM_modes(self):
        ...
        self.n_msh_pts, self.n_msh_el = map(
            int, tstruc.get_mail_data()[0].split())

        print('\n Structure has {0} mesh points and {1} mesh elements.'.format(
...
#      self.calc_EM_mode_energy()
    ...
    def calc_AC_modes(self, bcs=None):
        ...
        print(f'\n Structure has {self.n_msh_pts} mesh points and {self.n_msh_el} mesh elements.')
