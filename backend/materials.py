...
class Material(object):
    ...
    def _fill_4tensor_from_elt_patterns(self, tens, indep_elts, dep_elts):
        for i, j in indep_elts:
            tens.read_from_json(i, j)

        for dest,src in dep_elts.items():
            (dI,dJ) = dest
            for (sI,sJ,sf) in src:
    ...
    def construct_crystal_hexagonal(self):
        ...
        C_indep_elts = [(1, 1), (1, 2), (1, 3), (3,3), (4, 4)]
        C_dep_elts = {
                    (2,1): [(1,2,1)],
                    (2,2): [(1,1,1)],
                    (2,3): [(1,3,1)],
        ...
        try:
            ...
            # for lintens in [self.stiffness_c_IJ, self.viscosity_eta_IJ]:
            #     for i, j in C_indep_elts:
            #         lintens.read_from_json(i, j)

            #     for k,v in C_dep_elts.items():
            #         (dI,dJ) = k
            #         (sI,sJ,sf) = v
            #         lintens[dI, dJ] +=  lintens[sI,sJ] * sf
