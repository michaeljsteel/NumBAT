...
def _load_waveguide_templates(p_wgtemplate_dir, p_wgtemplate_index):
    ...
    try:
        ...
        raise Exception(
            "JSON parse error in reading user mesh index file" + str(ex)
        ) from ex
    ...
    for wg in wg_index:
        ...
        if not pth_mod.exists():
            ...
            reporting.report_and_exit(
                f"Missing waveguide template implementation file: {str(wgpy)} named in str({p_wgtemplate_index})."
                + f"\nFile was expected in directory {str(p_wgtemplate_dir)}"
            )

        spec = importlib.util.spec_from_file_location(
            wgnm, pth_mod
        )  # TODO: is wgnm the right name for this?
        try:
            py_mod = spec.loader.load_module()
        except Exception as ex:
            reporting.report_and_exit(
                f"Python couldn't load the user module '{str(wgpy)}'."
                + "\n Your code likely contains a syntax error or an attempt to load another module that was not successful."
                + f"\n\n The Python traceback contained the error message:\n   {str(ex)}."
                + f"\n\n\n The full traceback was {traceback.format_exc()}"
            )
        if not hasattr(py_mod, wgclsnm):
            reporting.report_and_exit(
                f"Can't find waveguide template class {wgclsnm} in implementation file: {wgpy}"
            )
        ...
        for k_typ in range(self.n_mats_ac):
            if v_acoustic_mats[k_typ]:
                ...
                if symmetry_flag:  # is it actually worth making this saving?
                    ...
                    for i in range(6):
                        for j in range(6):
                            self.c_IJ[i, j, k_typ] = t_ac_c_IJ[
                                i + 1, j + 1
                            ]  # TODO: replace with Voigt.value() ?
...
class Structure:
    ...
    def initialise_waveguide_templates(cls, nbapp):
        ...
        if not pmsh_index_builtin.exists():
            ...
            cls._waveguide_templates = _load_waveguide_templates(
                pmsh_dir, pmsh_index_builtin
            )
    ...
    def __init__(
        ...
        self.optical_props = OpticalProps(
            list(self.d_materials.values()), n_mats_em, self.loss
        )
    ...
    def _new_mesh_required(self):  # TODO: msh_name ? REMOVE ME
        ...
        return self.force_mesh or not os.path.exists(
            self.msh_location_in + msh_name + ".mail"
        )
    ...
    def build_waveguide_geometry(self, d_materials):
        ...
        for wg in Structure._waveguide_templates:
            if (
                self.inc_shape in wg["inc_shape"]
            ):  # is the desired shape supported by this template class?
                found = True
    ...
    def check_mesh(self):
        ...
        gmsh_cmd = (
            gmsh_exe + " " + str(self.msh_location_out) + "/" + self.msh_name + ".geo"
        )
        os.system(gmsh_cmd)

        gmsh_cmd = (
            gmsh_exe + " " + str(self.msh_location_out) + "/" + self.msh_name + ".msh"
        )
        os.system(gmsh_cmd)

    def calc_EM_modes(self, num_modes, wl_nm, n_eff, Stokes=False, debug=False, **args):
    ...
    def calc_AC_modes(
        self, num_modes, q_AC, shift_Hz=None, EM_sim=None, bcs=None, debug=False, **args
    ):
    ...
    def plot_refractive_index_profile(self, prefix, as_epsilon=False):
        ...
        v_regindex = scipy.interpolate.griddata(xy_in, v_refindex, xy_out).reshape(
            [n_pts_y, n_pts_x]
        )
        fig, ax = plt.subplots()
